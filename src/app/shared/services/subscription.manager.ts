...
export class SubscriptionManager {
  ...
  private readonly _subsHandler: { [key: string]: Subscription } = new Proxy(this._subs, {
    get: (subs: { [key: string]: Subscription }, prop: string) => {
      return subs[prop];
    },
    set: (subs: { [key: string]: Subscription }, prop: string, value: Subscription) => {
      if (subs[prop]) {
  ...
  public unsubscribe(): void {
    for (const prop in this._subs) {
      if (this._subs.hasOwnProperty(prop) && this._subs[prop] && this._subs[prop] instanceof Subscriber) {
